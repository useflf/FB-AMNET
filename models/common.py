...
class LogVarLayer(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.dim = dim  #计算数据沿给定维度（dim）的对数方差
...
class Conv2dSameExport(nn.Conv2d):
    ...
    # pylint: disable=unused-argument
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True):
        super(Conv2dSameExport, self).__init__(in_channels, out_channels, kernel_size, stride, 0, dilation, groups,
                                               bias)
        self.pad = None
...
class FBMSNet(nn.Module):
    ...
    def __init__(self,
        ...
        size = self.get_size(nChan, nTime, nBand)
    ...
    def get_size(self, nChan, nTime, nBand):
        data = torch.ones((1, nBand, nChan, nTime))
